...
<div class="row justy-content-center">
    ...
    <?php endif ?>
       ...
       <div class="card">
           <div class="card-body">
           	<a href="<?= base_url('tendik/tambah_berita_pengumuman') ?>" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Tambah</a>
               <table class="table table-stripped" id="dynamic-table">
               	...
               	<tbody>
               		<?php $no =1; foreach ($pengumuman as $p): ?>
               			<tr>
	               			<td><?= $no++ ?></td>
	               			<td width="100"><img src="<?= base_url('assets/adm/file_upload/'.$p->gambar) ?>" alt="img fasilitas" width="100"></td>
                      <td><?= $p->jenis ?></td>
                      <td><?= $p->judul ?></td>
                      <td width="40%"><?= $p->isi ?></td>
                      <td width="50px">
                        <a href="<?= base_url('tendik/hapus_pengumuman_berita/'.$p->id) ?>" title="hapus" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></a>
                      </td>   
	               		</tr>
               		<?php endforeach ?>
               	</tbody>

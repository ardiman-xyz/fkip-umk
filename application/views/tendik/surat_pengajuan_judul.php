  ...
  <div class="col-xs-12">
    ...
    <ul class="nav nav-pills">
     ...
     <!-- <div class="pull-right" style="margin-bottom: 10px; margin-top: 10px;">   
        <button onclick="bulk_delete('izin_penelitian')" class="btn btn-sm btn-danger btn-flat" type="button"><i class="fa fa-trash"></i> Delete</button>
      </div> -->
  </div>
  <?=form_open('',array('id'=>'form_sk_beasiswa'))?>
    <div class="table-responsive">
      <table id="dynamic-table" class="table table-striped table-bordered table-hover">
        ...
        <tbody>
          <?php $no = 1; foreach ($surat as $key => $data): ?>
            <tr>
              <td><?= $no++; ?></td>
              ...
              <td align="center">
                ...
                <?php else: ?>
                 ...
                 <span class="kirim" title="kirim ke <?= $data->nama ?>" id="kirim<?= $key+1 ?>" onclick="kirim_pesan('izin_penelitian',<?= $key+1 ?>, <?= $data->id ?>)">Send</span>
                <?php endif ?>
              </td>
              <td align="center">
                <?php if ($data->no_surat == 0): ?>
                  <input type="number" name="no_surat" class="form-control" id="no_surat<?= $key+1 ?>">
                   <span class="kirim" title="input no surat" id="add_no_surat<?= $key+1 ?>" onclick="add_no_surat('izin_penelitian',<?= $key+1 ?>, <?= $data->id ?>)">Add</span>
                <?php else: ?>
                  <?= $data->no_surat ?>
                <?php endif ?>
              </td>
              <td>
                <?php if ($data->file != ''): ?>
                    <a target="_blank" href="<?= base_url('assets/upload/surat_mahasiswa/'.$data->file) ?>" title=""><?= $data->file ?></a> &nbsp;<i class="fa fa-trash red" id="remove<?= $key+1 ?>" onclick="hapus_file('izin_penelitian',<?= $data->id ?>,<?= $key+1 ?>)" title="hapus file"></i>
                <?php else: ?>
                  <input type="file" name="file" id="fileUpload<?= $key+1 ?>" onchange="file_upload(<?= $data->id ?>,<?= $key+1 ?>)" <?= ($data->status != 'diterima' ? 'disabled' : '') ?>> 
                  <span id="msg<?= $key+1 ?>"></span>
                <?php endif ?>
              </td>
              <td>
                <div class=" action-buttons">
                  <select name="status" onchange="approve_surat('izin_penelitian', <?= $data->id ?>, $(this).val())">
                  <option value="">--pilih--</option>
                  <option value="diterima">Terima</option>
                  <option value="ditolak">Tolak</option>
                </select>
                  <a class="btn btn-xs btn-primary" target="_blank" title="download surat" href="<?= base_url('mahasiswa/surat/pdf/'.$data->id) ?>">Lihat</a>
                  <a href="<?= base_url('tendik/editSuratIzinPenelitian/'.$data->id) ?>" title="Edit" class="btn btn-xs btn-warning">Edit</a>
                </div>
              </td>
              <td>

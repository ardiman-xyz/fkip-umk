...
<div style="margin-top: 20px">
	...
	<?= form_open('tendik/edit_raw/'.$data->id_raw, ['id' => 'form_raw']); ?>
		<div class="col-md-9">
			...
			<div class="table-responsive">
				<table >
					<tr>
						<td>Input jumlah wisudawan : </td><td> &nbsp;<input type="number" name="jumlah_wisudawan" required value="<?= $data->jml_mhs ?>"></td>
					</tr>
				</table>
				<hr>
				<h5>
				...
				<table class="table-sm table table-bordered table-hover" id="myTable">
					...
					<tbody>
						<?php foreach ($pengelola as $key => $p): ?>
							<tr>
								<td><?= $key + 1 ?></td>
								<td>
									<input type="text" name="nama_pengelola[]" class="form-control" placeholder="Masukkan nama pengelola" onkeyup="cari_pengelola(<?= $key+1 ?>)" id="input_pengelola<?= $key+1 ?>" required value="<?= $p->nama_pengelola ?>">
								</td>
								<td class="text-center">
									<select name="jabatan[]" class="form-control" id="jabatan${i}" required>
										<option value="">--pilih jabatan--</option>
										<option value="ketua program studi" <?= ($p->jabatan === 'ketua program studi' ? 'selected' : '')?>>Ketua program studi</option>
										<option value="staff administrasi" <?= ($p->jabatan === 'staff administrasi' ? 'selected' : '')?>>Staff administrasi</option>
										<option value="staff akademik" <?= ($p->jabatan === 'staff akademik' ? 'selected' : '')?>>Staff akademik</option>
									</select>
								</td>
								<td class="text-center">
	    ...
	    $("#btn-tambah").on("click", function () {
	        ...
	        cols += `
	        	<tr>
					...
					<td class="text-center">
					<input type="text" name="nama_pengelola[]" class="form-control" placeholder="Masukkan nama pengelola" onkeyup="cari_pengelola(${i})" id="input_pengelola${i}" required>
					</td>
					<td class="text-center">
						<select name="jabatan[]" class="form-control" id="jabatan${i}" required>
							<option value="">--pilih jabatan--</option>
